...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process single-cell RNA and sgRNA data from a local MuData file.")
    parser.add_argument("--mudata_path", required=True, help="Path to local .h5mu file")
    ...
    # Derive a base name for output files from the input filename
    base_name = os.path.splitext(os.path.basename(mudata_path))[0]

    # Define output file paths
    final_path_name = os.path.join(out_dir, f"{base_name}.h5mu")
    ...
    # Copy the local file to the output directory (if not already there)
    if os.path.abspath(mudata_path) != os.path.abspath(final_path_name):
        print(f"Copying {mudata_path} to {final_path_name}...")
        shutil.copy(mudata_path, final_path_name)
    else:
        print("Input file is already in the output directory, skipping copy.")
