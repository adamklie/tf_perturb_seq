...
### 3. `3_guide_capture_comparison.ipynb` - Guide Representation

How guides are represented across technologies: cell counts, UMI distributions, overlap, and uniformity. Uses `per_guide_capture.tsv` files (generated by `extract_per_guide_capture.py`).

#### Section 1: Cell Counts per Guide

**`guide_cells_ranked_barplot.pdf`**
- Ranked bar charts (one per dataset) of cells per guide, sorted by count
- Median annotated with dashed line
- *Interpretation*: Shape reveals library uniformity. Steep drop-off = uneven representation.

**`guide_cells_violin.pdf`**
- 2-panel violin plots: raw cell counts (left) and fraction of total cells (right)
- *Interpretation*: Compare distributions across datasets. Fraction normalizes for dataset size.

#### Section 2: UMI Distributions per Guide

**`guide_total_umi_ranked_barplot.pdf`**
- Ranked bar charts of total UMI per guide for each dataset

**`guide_umi_violin.pdf`**
- 3-panel violin: total UMI, mean UMI per cell, median UMI per cell

**`guide_umi_violin_log10.pdf`**
- Same as above but log10-scaled for cross-dataset comparison

#### Section 3: Guide Overlap Across Datasets

**`guide_overlap_summary.pdf`**
- Left: Histogram of how many datasets each guide is detected in
- Right: Horizontal bar showing shared (all datasets) vs dataset-specific vs undetected

**`guide_detection_patterns.pdf`**
- Top detection patterns (which combinations of datasets share guides)

#### Section 4: Guide Coverage Uniformity

**`guide_uniformity_lorenz.pdf`**
- Lorenz curves for cell counts and total UMI per guide
- *Interpretation*: Closer to diagonal = more uniform library representation

**`guide_cells_correlation_matrix.pdf`**
- Pairwise scatter matrix of cells per guide across datasets
- Pearson r in each panel. High r = same guides are over/under-represented.

**`guide_umi_correlation_matrix.pdf`**
- Same as above for mean UMI per guide

**`guide_most_variable_heatmap.pdf`**
- Heatmap of top 20 most variable guides (by CV of cell detection fraction across datasets)

**`guide_top_bottom_captured.pdf`**
- Side-by-side heatmaps: top 10 most-captured and bottom 10 least-captured guides

#### Intermediate Files:
- `combined_per_guide_capture.tsv`: All per-guide capture stats combined
- `guide_frac_cells_matrix.tsv`: Guide x dataset matrix of cell detection fractions with CV
- `guide_detection_binary.tsv`: Binary detection matrix with overlap patterns
- `guide_cells_summary.tsv`: Summary statistics of cells per guide
- `guide_umi_summary.tsv`: Summary statistics of UMI per guide
