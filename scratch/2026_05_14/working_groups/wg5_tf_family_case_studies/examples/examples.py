...
Run from the jamboree root:
    ...
    uv run python working_groups/wg5_tf_family_case_studies/examples/examples.py
"""
...
def load_family_activity_scorecard():
    return pd.read_csv(
        JAMBOREE_ROOT / "working_groups/wg5_tf_family_case_studies/examples/family_activity_scorecard.tsv",
        sep="\t",
    )


def load_tf_metadata(simplified=False):
    name = "tf_metadata_simplified.tsv" if simplified else "tf_metadata.tsv"
    return pd.read_csv(JAMBOREE_ROOT / "reference" / name, sep="\t")


def load_jaspar_tf_metadata():
    return pd.read_csv(JAMBOREE_ROOT / "reference/jaspar_core_tf_metadata.tsv", sep="\t")
