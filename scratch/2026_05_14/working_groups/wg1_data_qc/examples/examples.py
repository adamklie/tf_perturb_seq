...
Run from the jamboree root:
    ...
    uv run python working_groups/wg1_data_qc/examples/examples.py
...
def load_qc_summary():
    return pd.read_csv(
        JAMBOREE_ROOT / "working_groups/wg1_data_qc/examples/qc_summary.tsv", sep="\t"
    )


def load_edistance_summary_wg1():
    return pd.read_csv(
        JAMBOREE_ROOT / "working_groups/wg1_data_qc/examples/edistance_summary.tsv", sep="\t"
    )


def load_tf_cross_lineage():
    return pd.read_csv(
        JAMBOREE_ROOT / "working_groups/wg1_data_qc/examples/tf_cross_lineage.tsv", sep="\t"
    )


def load_significant_tfs(dataset):
    full = DATASETS.get(dataset, dataset)
    return pd.read_csv(
        JAMBOREE_ROOT / "data" / full / "energy_distance" / "wg1_significant_tfs.tsv", sep="\t"
    )
