"""Standalone UMAP visualization for the meta-consensus pipeline.
...
UMAP(n_components=2) on it once, and writes:
  - Result/UMAP/umap_programs_by_dataset.png   (global, k-independent)
  - Result/UMAP/k_{k}/umap_programs_by_cluster.png  (per k)
"""
...
from umap_plots import (  # noqa: E402
    fit_umap,
    plot_umap_programs_by_cluster,
    plot_umap_programs_by_dataset,
)
...
def main() -> None:
    ...
    ap.add_argument(
        "--umap-output-dir",
        default="/oak/stanford/groups/engreitz/Users/ymo/Testing_Scripts/Meta_program/Result/UMAP",
        help="Where to write UMAP plots",
    )
    ...
    os.makedirs(args.umap_output_dir, exist_ok=True)
    ...
    print(
        f"\n# Fitting UMAP on input programs "
        f"(n_neighbors={args.n_neighbors}, min_dist={args.min_dist}, "
        f"metric={args.metric}, random_state={args.random_state})"
    )
    embedding, _ = fit_umap(
        N,
        n_components=2,
    ...
    global_out = os.path.join(args.umap_output_dir, "umap_programs_by_dataset.png")
    plot_umap_programs_by_dataset(
        embedding, N.index, global_out,
        title=f"UMAP of input programs ({N.shape[0]} programs, {N.shape[1]} genes)",
    )
    ...
    for k in meta_k_list:
        ...
        k_out_dir = os.path.join(args.umap_output_dir, f"k_{k}")
        os.makedirs(k_out_dir, exist_ok=True)
        out_png = os.path.join(k_out_dir, "umap_programs_by_cluster.png")
        plot_umap_programs_by_cluster(
            embedding, N.index, labels, out_png,
            title=f"UMAP of input programs colored by dataset (k={k})",
            annotate_centroids=args.annotate_clusters,
        )
        print(f"  k={k}: wrote {out_png}")

    print(f"\n# Done. Plots in {args.umap_output_dir}")
