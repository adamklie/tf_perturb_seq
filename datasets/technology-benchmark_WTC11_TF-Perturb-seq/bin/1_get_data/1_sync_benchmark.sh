...
#
# USAGE:
#   cd datasets/technology-benchmark_WTC11_TF-Perturb-seq/bin/1_get_data
#
#   ./1_sync_benchmark.sh \
...
#   ./1_sync_benchmark.sh --gcp-prefix gs://... --run-label sceptre_v11 --dry-run
#
#   # Skip download (already synced), regenerate manifests only
#   ./1_sync_benchmark.sh --gcp-prefix gs://... --run-label sceptre_v11 --skip-download
###############################################
...
BENCHMARK_DIR="$(dirname "$(dirname "${SCRIPT_DIR}")")"
...
for entry in "${DATASET_MAP[@]}"; do
  ...
  gcp_dashboard="${GCP_PREFIX}/${gcp_name}/pipeline_dashboard"
  gcp_mudata="${GCP_PREFIX}/${gcp_name}/pipeline_outputs/inference_mudata.h5mu"

  if gsutil ls "${gcp_dashboard}/" &>/dev/null; then
    AVAILABLE_DASHBOARD+=("${dataset_label}:${gcp_name}:${local_name}")
    echo "[DASHBOARD] ${gcp_name} → ${dataset_label}"
  elif gsutil ls "${gcp_mudata}" &>/dev/null; then
    AVAILABLE_OUTPUTS+=("${dataset_label}:${gcp_name}:${local_name}")
    echo "[OUTPUTS]   ${gcp_name} → ${dataset_label}  (no dashboard; run qc_array.sh after sync)"
  else
...
if [[ "${DRY_RUN}" -eq 1 ]]; then
  ...
  for entry in "${AVAILABLE_OUTPUTS[@]+"${AVAILABLE_OUTPUTS[@]}"}"; do
    ...
    local_run_dir="${DATASETS_ROOT}/${local_name}/runs/${RUN_LABEL}"
    printf "%s\t%s\t%s\n" \
      "${local_run_dir}/inference_mudata.h5mu" \
