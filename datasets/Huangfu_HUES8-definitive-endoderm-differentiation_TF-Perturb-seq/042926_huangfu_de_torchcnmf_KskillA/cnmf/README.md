# PerturbNMF — Huangfu HUES8 Definitive Endoderm

End-to-end PerturbNMF run on the Huangfu HUES8 endoderm differentiation dataset (~269,000 cells × ~36,000 genes, 14,151 guides → 2,063 unique targets including 600 NT controls). Run started 2026-04-29 (inference) and completed 2026-05-09 (Stage 3).
...
| Input h5ad | `Data/DE_muddy_penguin_perturbnmf.h5ad` |
...
3. For any TF of interest, open `Plot/Perturb_gene_200_2_0/<TF>.pdf` — volcano + UMAP + per-program log2FC + correlation waterfall. The merged PDF (`merged_perturbed_genes_DE_K200.pdf`, 583 MB) is all of these glued together.
...
| 2a. Evaluation — motif enrichment | ⏭️ Skipped — no scE2G enhancer-gene links for HUES8 endoderm |
...
| 3c. Per-target PDFs | ✅ | `Plot/Perturb_gene_200_2_0/<TF>.pdf` (1424 PDFs; 7 originally empty have been re-run) |
| 3c. Merged combined PDF | ✅ | `Plot/Perturb_gene_200_2_0/merged_perturbed_genes_DE_K200.pdf` (~580 MB) — built with `pdfunite` since upstream's PyPDF2 merge hangs |
...
├── README.md                                # this file
├── Data/
│   ├── DE_muddy_penguin_perturbnmf.h5ad     # inference input (raw counts)
...
├── Result/042926_huangfu_de_torchcnmf_KskillA/
...
└── Script/                                  # SLURM submission scripts (this dataset)
    ├── 042926_huangfu_de_torchcnmf_KskillA_inference.sh
