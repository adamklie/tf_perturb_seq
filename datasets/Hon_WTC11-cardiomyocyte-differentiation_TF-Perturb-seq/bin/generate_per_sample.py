...
def generate_per_sample_tsv(analysis_set_accession, output_path, auth, hash_seqspec=None, rna_seqspec=None, sgrna_seqspec=None, default_barcode_map=None):
    ...
    for input_file_set in input_file_sets:
        ...
        if modality == 'cell hashing barcode sequencing':
            ...
            if not(barcode_to_hashtag_map):
                # HACK (Hon cardio, 2026-04-15): portal aux sets missing barcode_map.
                # All hash rows for this dataset use IGVFFI3028BJFI — fall back to it.
                print(f'[WARN] Missing barcode_map on {file_set_object["@id"]} — falling back to IGVFFI3028BJFI')
                barcode_to_hashtag_map = 'IGVFFI3028BJFI'
            else:
