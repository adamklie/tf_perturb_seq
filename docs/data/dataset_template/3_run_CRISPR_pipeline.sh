...
# Sample metadata with GCS paths (update filename as needed)
# Use _patched.csv if you ran patch_files.sh
SAMPLE_METADATA=$BASE_DIR/sample_metadata_gcp_YYYY_MM_DD.csv  # <-- CHANGE DATE

# CRISPR Pipeline path (local clone of the pipeline repo)
PIPELINE_PATH=/Users/adamklie/Desktop/projects/CRISPR_Pipeline

# Output directory on GCS (update date to match upload date)
