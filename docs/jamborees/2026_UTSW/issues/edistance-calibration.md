...
| Run | Targets | Cells/target (median) | Targeting `distance_mean` median | NC `distance_mean` median | `pval_mean` median | NC count with `pval_mean=0` | Targeting > NC max |
|---|---:|---:|---:|---:|---:|---:|---:|
| HTv2 verified reference (Sara, syn74381183) | 64 | 2000 | **1.78** | _no NCs in run_ | (healthy) | _n/a_ | _n/a_ |
| **HTv2 testbed** (our run, 2026-05-10, syn74895081) | **65** | **2000** | **19.46** | _no NCs in run_ | **0.0316** (non-degenerate) | _n/a_ | _n/a_ |
| Huangfu DE (syn74883327) | 2267 | 2000 | **93.21** | 91.70 | **0** | **100 / 100** | **69 / 2163** (3.2%) |
| Huangfu ESC (syn74883475) | 2267 | 2000 | **527.33** | 528.25 | **0** | **100 / 100** | **83 / 2163** (3.8%) |

> **Strong evidence for the scale-driven hypothesis**: our own HTv2 testbed run on 2026-05-10 (same pipeline, same wrapper, same parameters as the Huangfu production runs — but a small dataset of 37k cells × 65 targets) produced a **non-degenerate `pval_mean` distribution** (median 0.0316). The pipeline + our wrapper are *not* fundamentally broken; the calibration issue scales with cell count and target count. This is a useful internal data point — anyone investigating Issue 1 should compare the HTv2 testbed `pval_edist_full.csv` (small-scale healthy) against the Huangfu production CSVs (large-scale broken) to localize what the pipeline is doing differently at scale.
