...
Energy distance is the headline statistic for "did knocking down this TF move the transcriptome." It collapses thousands of single-cell profiles into one number per perturbation. Companion: [`CRISPR.md`](CRISPR.md), [`CNMF.md`](CNMF.md).

For the pipeline reference (run commands, intermediate files), see [`../../../analysis/energy_dist/ENERGY_DISTANCE_OUTPUTS.md`](../../../analysis/energy_dist/ENERGY_DISTANCE_OUTPUTS.md). This doc is the **interpretation** layer.
...
1. **The top-distance TFs** for the cell type — these are candidate lineage-defining TFs. *[FILL IN — cite references for specific TFs per lineage]*
...
3. **The "lineage-specific" TFs** — TFs significant in only one cell type. Candidate lineage-defining regulators and often the most interpretable hits.
