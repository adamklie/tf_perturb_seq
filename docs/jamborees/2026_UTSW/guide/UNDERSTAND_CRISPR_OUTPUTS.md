...
## The three folders you'll see in a bundle

A canonical CRISPR-pipeline output bundle has three top-level folders:
...
We always upload all three together. If a dataset is missing `pipeline_info/`, it's an incomplete bundle — flag it.
...
The canonical "every cell + every guide + every gene" object. See [`DATA_FORMATS.md`](DATA_FORMATS.md) for how to open. Modalities:
...
- `intended_target_name` — the gene symbol of the TF a cell's guide targets (e.g. `SOX17`). NTC cells get `non_targeting`.
...
my_tf = trans[(trans["intended_target_name"] == "SOX17") & (trans["fdr_bh"] < 0.05)]
print(my_tf.shape)
...
1. **The TFs with the broadest trans effects** in the dataset. These are the lineage-master candidates. Pull the top 10–20 by trans-target count.
2. **Whether your favorite TF actually moved its target gene** (cis QC). If not, the trans signal isn't trustworthy.
3. **Whether you can recapitulate a known biology result** — e.g. SOX17 knockdown in endoderm should kill endoderm-identity markers. If yes, the experiment is grounded; if not, ask.
4. **TFs with strong trans effects but unexpected functions** — the deorphanization candidates. Cross-reference the trans-DE table with TF annotations (Lambert 2018, JASPAR).
