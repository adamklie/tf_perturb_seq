...
The full mapping (one Synapse path per dataset × output) lives in [`synapse_paths.tsv`](synapse_paths.tsv). Latest dataset-checkpoint snapshot: [`state/2026_05_09_state.tsv`](state/2026_05_09_state.tsv) (compare against the original [`state/2026_05_07_state.png`](state/2026_05_07_state.png) / [`state/2026_05_07_state.tsv`](state/2026_05_07_state.tsv) to see what's moved).
...
├── state/                            (state snapshots; 2026_05_07 historical, 2026_05_09 latest)
...
├── communications/                   (live participant-facing notes; FOR_EXPERIMENTALISTS.md)
├── working_groups/                   (per-WG roll-ups + runnable examples.py)
├── issues/                           (one report per blocker, conversation-ready)
└── datasets/
    └── <dataset_name>/
