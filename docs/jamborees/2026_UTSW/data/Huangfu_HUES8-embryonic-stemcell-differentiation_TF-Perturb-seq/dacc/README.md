# DACC-format files — Huangfu HUES8 Embryonic Stem Cell
...
|---|---|---|---|
| [`gene_universe.tsv`](gene_universe.tsv) | DACC `Gene Universe` | cNMF `Inference.overdispersed_genes.txt` from `sceptre_v1` run | 2,000 HVGs, all resolved to ENSG (0 unmapped). |
...
PYTHONPATH=src uv run python -m tf_perturb_seq.dacc.build_gene_universe \
    --hvg datasets/Huangfu_HUES8-embryonic-stemcell-differentiation_TF-Perturb-seq/sceptre_v1/cnmf/Result/Inference/Inference.overdispersed_genes.txt \
    --gtf ref/genome/IGVFFI9573KOZR.gtf.gz \
    --out docs/jamborees/2026_UTSW/datasets/Huangfu_HUES8-embryonic-stemcell-differentiation_TF-Perturb-seq/dacc/gene_universe.tsv \
    --misses-out docs/jamborees/2026_UTSW/datasets/Huangfu_HUES8-embryonic-stemcell-differentiation_TF-Perturb-seq/dacc/gene_universe.misses.tsv
```
