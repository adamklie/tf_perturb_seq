"""Mirror a dataset's CRISPR pipeline outputs from GCS to Synapse.
...
DISK SPACE: each dataset's pipeline output is ~60 GB. The default `/tmp/jamboree_pipeline_<id>`
works on a laptop only when the data fits. **On HPC where `/tmp` is typically small, pass
`--workdir <your-scratch>/jamboree_pipeline_staging/<id>` to stage on a roomy filesystem.
"""
