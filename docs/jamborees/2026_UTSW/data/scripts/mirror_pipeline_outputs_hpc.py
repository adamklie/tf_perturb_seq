"""Mirror a CRISPR pipeline output directory from a local HPC path to Synapse.

Companion to `mirror_pipeline_outputs.py` (GCS source). Use this variant when the
run lives on HPC rather than GCS — currently the case for the Gersbach HTv2
...
What gets uploaded:
  Expected layout (whichever subset is present):
...
Usage:
    python mirror_pipeline_outputs_hpc.py \\
        ...
        --source-dir <your-hpc-scratch>/HTv2/runs/cleanser_800_mito_15pc \\
...
PIPELINE_DIRS = ["pipeline_dashboard", "pipeline_info", "pipeline_outputs"]
...
def main() -> int:
    ...
    if not dirs_to_upload:
        sys.exit("No expected pipeline subdirs found under --source-dir. Aborting.")
